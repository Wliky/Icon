...
| **页面背景** | 设置里填一张图片 URL 当页面背景，可调遮罩浓度 / 暗纱亮纱 / 模糊度，只存本机 |
| **网格密度** | 图标网格支持大 / 中 / 小三档，随时切换并记住选择 |
...
点工具条的 **批量** 进入选择模式 → 勾选图标，然后可以：

- **打包下载 ZIP**：用的是 [JSZip](https://stuk.github.io/jszip/)（CDN 按需加载），加载不到时自动降级为逐个下载
- **批量删除**：一次删掉选中的全部图标，图片文件一起删。索引只在最后提交一次，删几十个也不会反复写仓库
...
设置 → 页面背景 → 填一张图片的 URL，即可把它铺成页面背景。下面三个滑块/开关可以调：

- **遮罩浓度**：数值越大背景越淡、文字越清楚
- **遮罩颜色**：暗纱（彩色壁纸推荐，对比更强）/ 亮纱
- **背景模糊**：给背景加模糊，内容更突出
