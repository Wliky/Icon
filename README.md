...
| | |
|---|---|
| **上传即用** | 图片原样保存，不压缩、不转格式 |
| **批量上传 / 下载 / 删除** | 一次拖多张；勾选后打包成 ZIP 下载，或一次性批量删除 |
...
点工具条的 **批量** 进入选择模式 → 勾选图标（按住 **Shift** 可连选一片）→ 然后：

- **打包下载 ZIP**：把选中的图标打包成一个 ZIP
- **删除**：一次性删掉选中的全部图标，图片文件一起删。索引只在最后提交一次，删几十个也不会反复写仓库
...
- **重命名**：只改显示名称，文件名和图片地址保持不变
- **删除**：连同仓库里的图片文件一起删除，无法撤销
